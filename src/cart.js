import {cartManagement, priceBeautify} from './Constant Fucntions/dataManipulations.js';
...
const getItems = async (cart) => {
    ...
    for (const [key, value] of Object.entries(cart)) {
        const data = await fetch(`https://fakestoreapi.com/products/${key}`)
...
const displayItems = (items) => {
    ...
    for (const [key, item] of Object.entries(items)) {
        ...
        `
        cartItem.querySelector('.item-quantity-changer').addEventListener('click', (e) => {
            ...
            if (e.target.innerHTML === "-") {
                ...
                if (quantity > 1) {
                    ...
                    item.quantity -= 1;
                    cartInLocal[key] -= 1;
                    cartItem.querySelector('.item-quantity-changer p').innerHTML = item.quantity;
                } else {
                    alert('You cannot have less than 1 item. please delete the item if you do not want it.');
                }
